...
class Elastic(object):
    ...
    def search_complex(self, body, field, num=100, fields_return="", start=0):
        """Searches in a given field using the similarity method configured in the index for that field.
        ...
        :return: dictionary of document IDs with scores
        """
        hits = self.__es.search(index=self.__index_name, body=body, df=field, _source=False, size=num,
                                from_=start)["hits"]["hits"]
        results = {}
        for hit in hits:
            results[hit["_id"]] = hit["_score"]
    ...
    def search_scroll(self, query, field, num=100, scroll="2m"):
        """Searches for the query in the given field using scrolling.
        Note: The scan search type disables sorting, to allow very efficient scrolling through large result sets.

        :param query: query string
        :param field: field to search in
        :param num: number of hits to return for each scroll
        :param scroll: scroll parameter; keep the search context open for another 2m
        :return: list of document IDs
        """
        # todo: This will hold scores for sentences
        # if hold_scores:
        #     page = self.__es.search(index=self.__index_name, q=query, df=field, scroll=scroll, search_type="scan",
        #                             size=num, params={'track_scores': 'true'})
        page = self.__es.search(index=self.__index_name, q=query, df=field, _source=False, scroll=scroll,
                                search_type="scan", size=num)

        sid = page["_scroll_id"]
        scroll_size = page["hits"]["total"]
        results = {}
        while scroll_size > 0:  # starts scrolling
            page = self.__es.scroll(scroll_id=sid, scroll=scroll)
            sid = page["_scroll_id"]  # updates the scroll ID
            hits = page["hits"]["hits"]
            scroll_size = len(hits)  # the number of results that we returned in the this scroll
            for hit in hits:
                results[hit["_id"]] = hit["_score"]
        return results
    ...
    # ================= Stats =================
    # =========================================
    def get_termvector(self, doc_id, field, term_stats=False):
    ...
    def __get_mtermvector(self, doc_list, field, term_stats=False):
        tv = self.__es.mtermvectors(index=self.__index_name, doc_type=self.DOC_TYPE, ids=doc_list, fields=field,
                                    term_statistics=term_stats)
        return tv.get("term_vectors", {}).get(field, {}).get("terms", {})
    ...
    def __get_coll_termvector(self, term, field):
        ...
        return self.get_termvector(doc_id, field, term_stats=True) if doc_id else {}
    ...
    def term_freqs(self, doc_id, field):
        ...
        """
        tv = self.get_termvector(doc_id, field)
        term_freqs = {}
        for term, val in tv.items():
            term_freqs[term] = val["term_freq"]
        return term_freqs

    def mterm_freqs(self, doc_list, field):
        tv = self.__get_mtermvector(doc_list, field)
        term_freqs = {}
        for term, val in tv.items():
            term_freqs[term] = val["term_freq"]
        return term_freqs
...
if __name__ == "__main__":
    ...
    # index_name = "wsdmcup17_wp_sentences_prof"
    # #field = "content"
    # field = "professions"
    # es = Elastic(index_name)
    # query = "accountant"
    # query1 = "singer-songwriter"
    # num = 100
    # a = es.scroll(query, field, num=1000, scr='2m', stype=False)
    # a1 = es.scroll(query1, field, num=1000, scr='2m', stype=False)
    # print(len(a),len(a1))
